...
if(!pq&&!id&&!lists&&!bf&&!skins&&!no){
	...
	if(pq=="1q"){
		...
		dz="http://www.6080tv.cn/cmp/player.swf?url=config.php";
		lb="http://www.6080tv.cn/"+lists+".xml"
	...
	var url="";
	if(!skins){
		url=dz+"&name="+encodeURIComponent("高清新片发布站")+"&description="+encodeURIComponent("WWW.XINPIAN.GQ")+"&announce_content="+encodeURIComponent("重要声明：所有影片均来源于其他影视站点，本站不制作和发布电影片源，不对内容的合法性承担任何责任！")+"&video_image=0&logo=0&ads=0&link=http://www.xinpian.gq&lists="+lb+"&.swf";
	}
	else{
		url=dz+"&name="+encodeURIComponent("高清新片发布站")+"&description="+encodeURIComponent("WWW.XINPIAN.GQ")+"&announce_content="+encodeURIComponent("重要声明：所有影片均来源于其他影视站点，本站不制作和发布电影片源，不对内容的合法性承担任何责任！")+"&video_image=0&logo=0&ads=0&link=http://www.xinpian.gq&skins="+skins+"&lists="+lb+"&.swf";
	};
	if(bf=="ck"){
